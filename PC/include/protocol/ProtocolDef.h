...
// Frame:
//   H1(0x55) H2(0xAA) Type(1) Len(1) Payload(Len) CRC16(2)  [CRC: Type..Payload dahil, H1/H2 dahil]
...
// ---- Sabitler ----
static constexpr uint8_t UART_H1 = 0x55;
static constexpr uint8_t UART_H2 = 0xAA;
...
// Örnek payload struct'ları (ihtiyaca göre genişletilebilir)
struct AimPayload {
    float pan_deg;
    float tilt_deg;
    float distance_m;
...
// ---- CRC16/CCITT-FALSE (table-driven) ----
...
static inline uint16_t crc16_ccitt_false_update(uint16_t crc, uint8_t data) {
    const uint8_t idx = static_cast<uint8_t>(((crc >> 8) ^ data) & 0xFFu);
    return static_cast<uint16_t>((crc << 8) ^ CRC16_CCITT_TABLE[idx]);
}

static inline uint16_t crc16_ccitt_false(const uint8_t* data, size_t len, uint16_t init) {
    uint16_t crc = init;
...
static inline uint16_t crc16_ccitt_false(const uint8_t* data, size_t len) {
    return crc16_ccitt_false(data, len, 0xFFFFu);
}
